...
class PointTNT(nn.Module):
    ...
    def __init__(
        self,
        #num_classes,
        local_attention = True, global_attention = True, n_anchor = 192, n_neigh = 20, dilation = 1, emb_dims=1024,
        channels=3,
    ):
        ...
        for _ in range(depth):
            ...
            layers.append(nn.ModuleList([
                ...
                PreNorm(point_dim, Attention(dim=point_dim, heads=heads, dim_head=dim_head,
                        dropout=attn_dropout)) if local_attention else nn.Identity(),
                PreNorm(point_dim, FeedForward(
                    dim=point_dim, dropout=ff_dropout)),
                ...
                PreNorm(patch_dim, Attention(dim=patch_dim, heads=heads, dim_head=dim_head,
                        dropout=attn_dropout)) if global_attention else nn.Identity(),
